...
async fn mm_login_success() {
    ...
    // Check Token header
    let token = response.headers().get("Token").expect("Missing Token header");
...
async fn mm_posts_flow() {
    ...
    // Register
    let reg_res = app.api_client.post(format!("{}/api/v1/auth/register", &app.address))
        .json(&user_data).send().await.unwrap();
    ...
    // Check my teams
    let teams_res = app.api_client.get(format!("{}/api/v4/users/me/teams", &app.address))
    ...
    let post_res = app.api_client.post(format!("{}/api/v4/posts", &app.address))
        .header("Authorization", format!("Bearer {}", token))
        .json(&post_data).send().await.unwrap();
    ...
    // Fetch posts
    let posts_res = app.api_client.get(format!("{}/api/v4/channels/{}/posts", &app.address, channel_id))
        .header("Authorization", format!("Bearer {}", token))
        .send().await.unwrap();
    ...
    assert_eq!(posts_body["posts"][post_id]["message"], "Hello from MM Mobile");
}
