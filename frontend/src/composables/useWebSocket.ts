...
function updateConnectionStatus() {
  ...
  if (
    disconnectedMs >= STATE_TRANSITION_MS.TO_FAILED ||
    reconnectAttempt.value >= MAX_RECONNECT_ATTEMPTS
  ) {
...
function normalizeWsPost(data: unknown, envelopeChannelId?: string): Post | null {
  ...
  const createdAt = normalizeWsTimestamp(
    normalizedRawPost.created_at ?? normalizedRawPost.create_at,
    fallbackTimestamp
  )
  const updatedAt = normalizeWsTimestamp(
    normalizedRawPost.updated_at ?? normalizedRawPost.update_at,
    createdAt
  )
  const postId = normalizeEntityId(normalizedRawPost.id) ?? (normalizedRawPost.id as string)
  const channelId =
    normalizeEntityId(normalizedRawPost.channel_id) ??
    normalizeEntityId(envelopeChannelId) ??
    (normalizedRawPost.channel_id as string | undefined) ??
    envelopeChannelId
  const userId =
    normalizeEntityId(normalizedRawPost.user_id) ??
    (normalizedRawPost.user_id as string | undefined)
  const rootPostId =
    normalizeEntityId(normalizedRawPost.root_post_id ?? normalizedRawPost.root_id) ??
  ...
  return {
    ...
    file_ids: Array.isArray(normalizedRawPost.file_ids)
      ...
      : [],
    is_pinned:
      typeof normalizedRawPost.is_pinned === 'boolean' ? normalizedRawPost.is_pinned : false,
...
function normalizeWsEnvelope(envelope: WsEnvelope): WsEnvelope {
  const broadcastChannelId =
    normalizeEntityId(envelope.broadcast?.channel_id) ?? envelope.broadcast?.channel_id
  const channelId =
    normalizeEntityId(envelope.channel_id) ?? envelope.channel_id ?? broadcastChannelId
...
function normalizeWsReactionPayload(data: unknown): Record<string, unknown> | null {
  ...
  const emojiName =
    typeof normalized.emoji_name === 'string'
...
function normalizeWsChannelPayload(data: unknown): Channel | null {
  ...
  const displayName =
    typeof dataObj.display_name === 'string' && dataObj.display_name
  ...
  return {
    ...
    creator_id:
      normalizeEntityId(dataObj.creator_id) ?? (dataObj.creator_id as string | undefined) ?? '',
  ...
  function applyInitialLoadSnapshot(data: unknown) {
    ...
    if (Array.isArray(dataObj.statuses)) {
      dataObj.statuses.forEach((rawStatus: unknown) => {
        ...
        const userId =
          normalizeEntityId(statusData.user_id) ?? (statusData.user_id as string | undefined)
  ...
  function handleMessage(envelope: WsEnvelope) {
    ...
    switch (envelope.event) {
      ...
      case 'thread_reply_created': {
        ...
        if (
          post.channel_id !== channelStore.currentChannelId &&
          post.user_id !== authStore.user?.id
        ) {
      ...
      case 'typing': // Compatibility with some mobile clients
        if (envelope.data && typeof envelope.data === 'object') {
          ...
          const typingChannelId =
            envelope.channel_id || envelope.broadcast?.channel_id || dataObj.channel_id
          if (!typingChannelId) {
            break
          }
      ...
      case 'unread_counts_updated': {
        if (envelope.data) {
          unreadStore.handleUnreadUpdate(
            envelope.data as { channel_id: string; team_id: string; unread_count: number }
          )
        }
        break
      }
