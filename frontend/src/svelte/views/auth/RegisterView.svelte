<script lang="ts">
  ...
  async function handleRegister() {
    ...
    try {
      ...
      const data = await response.json()
      registeredEmail = data.email ?? email
      success = true
...
<svelte:head>
  <title>Create account - RustChat</title>
</svelte:head>

<main class="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
  <section class="sm:mx-auto sm:w-full sm:max-w-md" aria-labelledby="register-title">
    <h1 id="register-title" class="text-center text-3xl font-bold tracking-tight text-gray-900">
      Create your RustChat account
    </h1>
      ...
      {#if success}
        <div class="text-center py-8">
          ...
          <p class="text-gray-600 mb-6">
            We've sent a password setup link to <strong>{registeredEmail}</strong>. Please check your inbox and click the link to complete your registration.
          </p>
      ...
      {:else}
        <form class="space-y-6" on:submit|preventDefault={handleRegister}>
          ...
          <div>
            ...
            <input id="username" name="username" required placeholder="jdoe" bind:value={username} class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
          </div>
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
            <p class="text-sm text-blue-700">You'll receive an email with a link to set your password after registration.</p>
          </div>
