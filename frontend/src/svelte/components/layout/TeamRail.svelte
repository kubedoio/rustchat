  ...
  {#each teams as team (team.id)}
    ...
    >
      ...
      {#if currentTeamId !== team.id && ($uiStore.unreadCountsByTeam?.[team.id] ?? 0) > 0}
        <span class="absolute -top-0.5 -right-0.5 w-3 h-3 bg-danger rounded-full border-2 border-bg-surface-2" />
      {/if}
      {#if currentTeamId === team.id}
        <span class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-brand rounded-r-full" />
      {/if}
    </button>
  {/each}
