...
export const useUnreadStore = defineStore('unreadStore', () => {
  ...
  function applyPostUnread(data: ChannelUnreadAt) {
    channelMentions.value[data.channel_id] = Number.isFinite(data.mention_count)
      ? data.mention_count
      : 0
  ...
  const totalUnreadCount = computed(() =>
    Object.values(channelUnreads.value).reduce((a, b) => a + b, 0)
  )
  const totalMentionCount = computed(() =>
    Object.values(channelMentions.value).reduce((a, b) => a + b, 0)
  )
  const getChannelUnreadCount = computed(
    () => (channelId: string) => channelUnreads.value[channelId] || 0
  )
  const getTeamUnreadCount = computed(() => (teamId: string) => teamUnreads.value[teamId] || 0)
  const getChannelReadState = computed(
    () => (channelId: string) => channelReadStates.value[channelId]
  )
