...
export const useChannelPreferencesStore = defineStore('channelPreferences', () => {
  ...
  const isFavorite = computed(() => (channelId: string) => favoriteChannels.value.has(channelId))

  // Check if a channel is muted
  const isMuted = computed(() => (channelId: string) => mutedChannels.value.has(channelId))

  // Get notify props for a channel
  const getNotifyProps = computed(
    () => (channelId: string) => channelNotifyProps.value[channelId] || {}
  )
