  ...
  <div class="space-y-8">
    ...
    <div class="rounded-r-3 border border-border-1 bg-bg-surface-1 p-6 shadow-1">
      ...
      <p class="mt-2 max-w-2xl text-sm text-text-3">
        Monitor your RustChat instance health, usage, and realtime capacity from a calmer
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-5">
      ...
      >
        <div class="mb-4 flex items-center justify-between">
          ...
          <div
            :class="[
              ...
              'flex h-11 w-11 items-center justify-center rounded-r-2 border',
            ]"
          >
        ...
        <p class="text-sm text-text-3">
          Current snapshot of {{ stat.label.toLowerCase() }} across your workspace.
        </p>
      </div>
    </div>
    ...
    <div class="rounded-r-3 border border-border-1 bg-bg-surface-1 p-6 shadow-1">
      <div class="mb-4">
        ...
        <p class="mt-1 text-sm text-text-3">
          Critical services should stay readable at a glance without the dashboard shouting at you.
        </p>
      </div>
      ...
      <div v-if="adminStore.health" class="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div class="rounded-r-2 border border-border-1 bg-bg-surface-2 p-4">
          <div class="flex items-center gap-3">
            <div
              :class="[
                healthToneClass(adminStore.health.database.connected),
            ...
            <div>
              ...
              <p class="text-sm text-text-3">
                {{
                  adminStore.health.database.connected
                    ? `${adminStore.health.database.latency_ms}ms latency`
                    : 'Disconnected'
                }}
              </p>
            </div>
        ...
        <div class="rounded-r-2 border border-border-1 bg-bg-surface-2 p-4">
          <div class="flex items-center gap-3">
            <div
              :class="[
                healthToneClass(adminStore.health.storage.connected),
                'flex h-10 w-10 items-center justify-center rounded-full border',
              ]"
            >
        ...
        <div class="rounded-r-2 border border-border-1 bg-bg-surface-2 p-4">
          <div class="flex items-center gap-3">
            <div
              class="flex h-10 w-10 items-center justify-center rounded-full border border-secondary/15 bg-secondary/10 text-secondary"
            >
      ...
      <div
        v-else
        class="rounded-r-2 border border-border-1 bg-bg-surface-2 py-8 text-center text-text-3"
      >
      ...
      <dl class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
        <div class="rounded-r-2 border border-border-1 bg-bg-surface-2 p-4">
          ...
          <dd class="mt-2 font-semibold text-text-1">
            {{ adminStore.health?.version || 'v2.0.0' }}
          </dd>
        </div>
        <div class="rounded-r-2 border border-border-1 bg-bg-surface-2 p-4">
          ...
          <dd class="mt-2 font-semibold text-text-1">
            {{
              adminStore.health?.uptime_seconds
                ? Math.floor(adminStore.health.uptime_seconds / 3600) + 'h'
                : '—'
            }}
          </dd>
        </div>
