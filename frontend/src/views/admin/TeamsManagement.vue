...
async function deleteTeam(team: AdminTeam) {
  if (
    !confirm(
      `Are you sure you want to delete the team "${team.display_name || team.name}"? This will permanently delete all channels, messages, and files.`
    )
  ) {
    return
  }
  ...
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-text-3 text-xs mt-0.5">
          {{
            selectedTeam
              ? 'Manage channels and members for this team'
    ...
    <div v-if="!selectedTeam" class="space-y-4">
      ...
      <div class="bg-bg-surface-1 rounded-xl border border-border-1 overflow-hidden shadow-sm">
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="bg-bg-surface-2 border-b border-border-1">
                <th
                  class="px-4 py-3 text-[10px] font-semibold text-text-3 uppercase tracking-wider"
                >
                  Team
                </th>
                <th
                ...
                <th
                  class="px-4 py-3 text-[10px] font-semibold text-text-3 uppercase tracking-wider text-center"
                >
                  Members
                </th>
                <th
                ...
                <th
                  class="px-4 py-3 text-[10px] font-semibold text-text-3 uppercase tracking-wider text-right"
                >
                  Actions
                </th>
              </tr>
            ...
            <tbody class="divide-y divide-border-1">
              ...
              >
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg bg-brand/10 flex items-center justify-center text-brand text-xs font-bold"
                    >
                ...
                <td class="px-4 py-3">
                  <span
                    v-if="team.is_public"
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-success/10 text-success border border-success/20"
                  >
                    <Globe class="w-3 h-3" />
                    Public
                  </span>
                  <span
                    v-else
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-primary/10 text-primary border border-primary/20"
                  >
                    <Lock class="w-3 h-3" />
                    Private
                  </span>
        ...
        <!-- Pagination -->
        <div
          class="px-4 py-3 flex items-center justify-between border-t border-border-1 bg-bg-surface-2/30"
        >
          <div class="text-xs text-text-3">
            Showing {{ (page - 1) * perPage + 1 }} to {{ Math.min(page * perPage, totalTeams) }} of
            {{ totalTeams }} teams
          </div>
        ...
        <div v-if="activeTab === 'channels'" class="p-5">
          ...
          <div v-else class="divide-y divide-border-1 border border-border-1 rounded-lg">
            <div
              ...
              :key="channel.id"
              class="px-4 py-3 flex items-center justify-between hover:bg-bg-surface-2/50 transition-colors"
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-medium text-text-1 text-xs flex items-center gap-2">
                    ...
                    <span
                      ...
                      class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-bg-surface-2 text-text-3"
                      >Archived</span
                    >
                  </div>
        ...
        <div v-if="activeTab === 'members'" class="p-5">
          ...
          <div v-else class="divide-y divide-border-1 border border-border-1 rounded-lg">
            ...
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-8 h-8 rounded-full bg-bg-surface-2 flex items-center justify-center text-text-2 text-xs font-bold"
                >
                  {{ (member.display_name || member.username).charAt(0).toUpperCase() }}
                </div>
                <div>
                  <div class="font-medium text-text-1 text-xs">
      ...
      <div class="p-5 space-y-4">
        ...
        <div class="space-y-2">
          <label class="flex items-center gap-2 text-xs text-text-2">
            <input
              v-model="teamForm.is_public"
              type="checkbox"
              class="w-4 h-4 text-brand rounded border-border-1"
            />
            <span>Public team (visible to all users)</span>
          </label>
          <label class="flex items-center gap-2 text-xs text-text-2">
            <input
              v-model="teamForm.allow_open_invite"
              type="checkbox"
              class="w-4 h-4 text-brand rounded border-border-1"
            />
    ...
    <BaseModal v-model="showCreateChannelModal" title="Create Channel">
      <div class="p-5 space-y-4">
        ...
        <BaseInput
          v-model="channelForm.display_name"
          label="Display Name"
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3 py-2 text-xs border border-border-1 rounded-lg bg-bg-surface-2 text-text-1 focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
          >
    ...
    <BaseModal v-model="showAddMemberModal" title="Add Team Member">
      <div class="p-5 space-y-4">
        ...
        <div class="max-h-60 overflow-y-auto space-y-1">
          <div v-if="searchingMembers" class="text-center py-4 text-xs text-text-3">
            Searching...
          </div>
          <div
            v-else-if="memberSearchResults.length === 0"
            class="text-center py-4 text-xs text-text-3"
          >
          ...
          >
            <div class="flex items-center gap-2.5">
              <div
                class="w-7 h-7 rounded-full bg-bg-surface-2 flex items-center justify-center text-text-2 font-bold text-xs"
              >
                {{ (user.display_name || user.username).charAt(0).toUpperCase() }}
              </div>
              <div>
                <div class="text-xs font-medium text-text-1">
