...
<template>
  <div class="flex h-screen bg-bg-surface-2">
    ...
    <aside class="w-56 bg-bg-surface-1 flex flex-col shrink-0 border-r border-border-1">
      ...
      <div class="h-14 flex items-center px-4 border-b border-border-1">
        <div class="flex items-center gap-2.5 min-w-0">
          ...
          <span class="font-semibold text-sm text-text-1 truncate">{{
            configStore.siteConfig.site_name || 'RustChat'
          }}</span>
          <span
            class="text-[10px] bg-brand/10 text-brand px-1.5 py-0.5 rounded font-medium shrink-0"
            >Admin</span
          >
        </div>
      </div>
    ...
    <main class="flex-1 flex flex-col min-w-0">
      <!-- Top Bar -->
      <header
        class="h-14 bg-bg-surface-1 border-b border-border-1 flex items-center justify-between px-6 shrink-0"
      >
        <h1 class="text-sm font-semibold text-text-1">{{ activeItemName }}</h1>
        <div class="flex items-center gap-3">
          <div
            class="w-7 h-7 rounded-full bg-brand/10 flex items-center justify-center text-brand text-xs font-bold"
          >
