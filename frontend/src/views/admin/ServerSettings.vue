...
onMounted(async () => {
  ...
  syncDefaultChannelsFromConfig(
    adminStore.config?.experimental as Record<string, unknown> | undefined
  )
})

watch(
  () => adminStore.config?.site,
  site => {
    if (site) {
...
<template>
  <div class="space-y-6">
    ...
    <div
      class="bg-bg-surface-1 rounded-xl shadow-sm border border-border-1 divide-y divide-border-1"
    >
      <!-- Site Information -->
      <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label class="block text-sm font-medium text-text-2 mb-1"
              >Logo URL (50x50 recommended)</label
            >
            <input
      ...
      <div class="p-6">
        ...
        <div class="max-w-xs">
          <label class="block text-sm font-medium text-text-2 mb-1"
            >Max Simultaneous Connections per User</label
          >
          <input
      ...
      <div class="p-6">
        ...
        <div v-if="showAdvanced" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label class="block text-sm font-medium text-text-2 mb-1"
              >Android App Download Link</label
            >
            <input
          ...
          <div>
            ...
            <p class="mt-1 text-xs text-text-3">
              Use <code>-1</code> for unlimited editing, <code>0</code> to disable editing, or a
              positive number for a time window.
            </p>
          </div>
        </div>
      ...
      <div class="p-6">
        ...
        <div>
          ...
          <p class="mt-2 text-sm text-text-3">
            Comma-separated channel names. Leave empty to use the fallback (<code>town-square</code>
            + <code>off-topic</code>). <code>town-square</code> is always included.
          </p>
        </div>
      </div>
