...
watch(
  () => adminStore.config?.integrations,
  integrations => {
    if (integrations) {
  ...
  <div class="space-y-6">
    ...
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div
          v-if="form.enable_webhooks"
          class="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-200"
        >
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Max per Team</label>
            <input
              v-model.number="form.max_webhooks_per_team"
              type="number"
          ...
          <div>
            ...
            <input
              v-model.number="form.webhook_payload_size_kb"
              type="number"
              class="w-full px-3 py-2 border rounded-lg"
            />
          </div>
