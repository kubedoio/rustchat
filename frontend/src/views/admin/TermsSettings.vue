...
<template>
  <div class="space-y-5">
    ...
    <div
      v-if="error"
      class="flex items-center gap-2 p-3 bg-danger/10 border border-danger/20 rounded-lg text-danger text-xs"
    >
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="px-1.5 py-0.5 bg-brand text-white text-[9px] rounded font-medium"
                >Active</span
              >
            </div>
            <p class="text-[10px] text-text-3 mt-1">
              Version {{ currentTerms.version }} • Effective
              {{ formatDate(currentTerms.effective_date) }}
            </p>
            <p v-if="currentTerms.summary" class="text-xs text-text-2 mt-2">
        ...
        >
          <div class="flex items-start gap-3">
            <div
              ...
              :class="terms.is_active ? 'bg-brand/10' : 'bg-bg-surface-2'"
            >
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span
                  ...
                  class="px-1.5 py-0.5 bg-brand text-white text-[9px] rounded font-medium"
                  >Active</span
                >
              </div>
              <p class="text-[10px] text-text-3 mt-0.5">
                Version {{ terms.version }} • Created {{ formatDate(terms.created_at) }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-1">
    ...
    >
      ...
      >
        <div class="flex items-center justify-between px-5 py-4 border-b border-border-1">
          <h3 class="text-sm font-bold text-text-1">
    ...
    <!-- Preview Modal -->
    <div
      v-if="showPreviewModal && editingTerms"
      class="fixed inset-0 z-50 flex items-center justify-center p-4"
    >
    ...
    <div v-if="showPendingModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        class="relative bg-bg-surface-1 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col"
      >
        <div class="flex items-center justify-between px-5 py-4 border-b border-border-1">
          <div>
            <h3 class="text-sm font-bold text-text-1">Users Pending Acceptance</h3>
            <p class="text-[10px] text-text-3">
              {{ pendingUsers.length }} users haven't accepted the terms
            </p>
          </div>
          <button
        ...
        <div class="overflow-y-auto max-h-[60vh]">
          ...
          <div v-else class="divide-y divide-border-1">
            ...
            >
              <div
                class="w-8 h-8 rounded-full bg-brand/10 flex items-center justify-center text-brand text-xs font-bold"
              >
