...
<template>
  <div class="space-y-6">
    ...
    >
      ...
      <div>
        ...
        <p class="text-sm text-danger/80 mt-1">
          The membership policy failure rate is {{ summary.failure_rate_24h.toFixed(1) }}% in the
          last 24 hours. Please review the recent failures below.
        </p>
      </div>
    </div>
    ...
    <div
      ...
      class="bg-bg-surface-1 rounded-lg shadow-sm border border-border-1"
    >
          ...
          <div class="flex items-start justify-between">
            <div>
              ...
              <p class="text-xs text-text-3 mt-1">
                User: @{{ failure.username || failure.user_id.slice(0, 8) }} | Target:
                {{ failure.target_type }} {{ failure.target_id.slice(0, 8) }}...
              </p>
      ...
      <table class="w-full">
        <thead class="bg-bg-surface-2">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-text-3 uppercase">
              Last Error
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-border-1">
          <tr v-for="stat in policyStats" :key="stat.policy_id" class="hover:bg-bg-surface-2">
            ...
            <td class="px-6 py-4 text-sm text-right text-danger font-semibold">
            ...
            <td
              class="px-6 py-4 text-xs text-text-3 truncate max-w-xs"
              :title="stat.last_error_message"
            >
