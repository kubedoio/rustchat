    ...
    <div class="flex items-center gap-3">
      ...
      <label class="inline-flex items-center gap-2 text-xs text-text-2">
        <input
          ...
          type="checkbox"
          class="w-3.5 h-3.5 text-brand rounded border-border-1"
        />
        Show deleted
      </label>
    </div>
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                  <div class="text-xs font-medium text-text-1 flex items-center gap-1.5">
                    ...
                    <span
                      v-if="isDeleted(user)"
                      class="px-1.5 py-0.5 text-[9px] rounded bg-danger/10 text-danger font-medium shrink-0"
                    >
                      Deleted
                    </span>
                  </div>
                  ...
                  <div
                    v-if="isDeleted(user) && user.delete_reason"
                    class="text-[9px] text-danger truncate"
                  >
                    {{ user.delete_reason }}
                  </div>
                </div>
    ...
    <BaseModal v-if="deletingUser" v-model="showDeleteModal" size="lg" @close="closeDeleteModal">
      ...
      <div class="p-5 space-y-4">
        <div class="rounded-lg border border-danger/20 bg-danger/10 p-3 text-xs text-danger">
          <p>
            <strong>Consequences:</strong> login is blocked immediately, active sessions are
            revoked, and the account is marked deleted for audit/history. Messages remain for
            history.
          </p>
        </div>
        ...
        <div class="text-xs text-text-2">
          <div>
            Type the exact <span class="font-semibold">username</span> or
            <span class="font-semibold">email</span> to confirm deletion.
          </div>
    ...
    <BaseModal v-if="wipingUser" v-model="showWipeModal" size="lg" @close="closeWipeModal">
      ...
      <div class="p-5 space-y-4">
        <div class="rounded-lg border border-warning/20 bg-warning/10 p-3 text-xs text-warning">
          <p>
            <strong>Warning:</strong> This will permanently remove the user record from the
            database. Only users with no messages can be wiped.
          </p>
        </div>
        ...
        <div class="text-xs text-text-2">
          ...
          <p class="mt-2 text-[10px] text-text-3">
            Deleted at: {{ formatDate(wipingUser.deleted_at ?? null) }}
          </p>
        </div>
          ...
          <div class="grid grid-cols-3 gap-3 text-center">
            ...
            <div class="rounded-lg bg-bg-surface-1 p-2">
              <div class="text-xl font-bold text-success">
                {{ resyncResult.memberships_applied }}
              </div>
            ...
            <div class="rounded-lg bg-bg-surface-1 p-2">
              <div
                ...
                :class="resyncResult.memberships_failed > 0 ? 'text-danger' : 'text-success'"
              >
                {{ resyncResult.memberships_failed }}
              </div>
              <div
                class="text-[10px]"
                :class="resyncResult.memberships_failed > 0 ? 'text-danger' : 'text-success'"
