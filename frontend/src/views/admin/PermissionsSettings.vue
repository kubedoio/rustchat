...
<template>
  <div class="space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
        ...
        >
          <div
            ...
            :class="activeRole === role.value ? 'text-brand' : 'text-text-1'"
          >
            {{ role.label }}
          </div>
    ...
    <div
      v-if="saveSuccess"
      class="flex items-center gap-2 p-3 bg-success/10 border border-success/20 rounded-lg text-success text-xs"
    >
      ...
      <div v-else class="divide-y divide-border-1">
        <div v-for="[category, items] in grouped" :key="category" class="p-5">
          <div class="flex items-center justify-between mb-3">
            <h3 class="text-xs font-semibold uppercase text-text-3 tracking-wider">
              {{ category }}
            </h3>
