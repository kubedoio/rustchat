...
const rhsStateByChannel = useStorage<Record<string, { view: RhsView; contextId?: string }>>(
  'rhs_state_by_channel',
  {}
)
    ...
    if (newId) {
      ...
      if (savedState?.view) {
        if (savedState.view === 'thread') {
          ...
          if (
            !savedState.contextId ||
            !messages.some(message => message.id === savedState.contextId)
          ) {
