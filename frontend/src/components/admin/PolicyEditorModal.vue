...
import { X, Trash2, Building2, Hash, AlertCircle } from 'lucide-vue-next'
...
const channels = ref<
  { id: string; name: string; display_name: string | null; team_id: string; channel_type: string }[]
>([])
...
const sourceTypeOptions = computed(() => {
  ...
  return [
    {
      value: 'all_users' as PolicySourceType,
      label: 'All Users',
      description: 'Apply to all users',
    },
    {
      value: 'auth_service' as PolicySourceType,
      label: 'Auth Service',
      description: 'Users from specific auth provider',
    },
    {
      value: 'group' as PolicySourceType,
      label: 'Group',
      description: 'Members of specific groups',
    },
    { value: 'role' as PolicySourceType, label: 'Role', description: 'Users with specific roles' },
    {
      value: 'org' as PolicySourceType,
      label: 'Organization',
      description: 'Users from specific organization',
    },
  ]
})
...
const groupNamesInput = computed({
  ...
  set: (val: string) => {
    sourceConfig.value.group_names = val
      .split(',')
      .map(s => s.trim())
      .filter(Boolean)
  },
...
const orgIdsInput = computed({
  ...
  set: (val: string) => {
    sourceConfig.value.org_ids = val
      .split(',')
      .map(s => s.trim())
      .filter(Boolean)
  },
...
<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="cancel">
    <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
      ...
      <div class="flex-1 overflow-auto p-6 space-y-6">
        <!-- Basic Info -->
        <div class="space-y-4">
          <h3 class="text-sm font-medium text-gray-900 uppercase tracking-wider">
            Basic Information
          </h3>
        ...
        <!-- Source -->
        <div class="space-y-4">
          <h3 class="text-sm font-medium text-gray-900 uppercase tracking-wider">Applies To</h3>
          ...
          <div
            v-if="sourceType === 'auth_service'"
            class="pl-4 border-l-2 border-indigo-200 space-y-3"
          >
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">
                Leave empty to match users from any auth provider
              </p>
            </div>
          </div>
          ...
          <div v-if="sourceType === 'group'" class="pl-4 border-l-2 border-indigo-200 space-y-4">
            <div>
              ...
              <div
                v-else
                class="space-y-2 max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-2"
              >
                <label
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :class="{
                    'bg-indigo-50 border-indigo-500': (sourceConfig.roles || []).includes(role),
                  }"
                >
                  <input
                    type="checkbox"
          ...
          <div v-if="sourceType === 'org'" class="pl-4 border-l-2 border-indigo-200 space-y-3">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1"> Organization IDs </label>
              <input
                v-model="orgIdsInput"
                type="text"
        ...
        <div class="space-y-4">
          ...
          <div v-else class="space-y-4">
            ...
            <!-- Available Teams -->
            <div>
              <h4 class="text-sm font-medium text-gray-700 mb-2">Add Team</h4>
              <div class="flex flex-wrap gap-2">
                <button
            ...
            <!-- Available Channels -->
            <div>
              <h4 class="text-sm font-medium text-gray-700 mb-2">Add Channel</h4>
              <div class="flex flex-wrap gap-2">
                <button
        ...
        <!-- Settings -->
        <div class="space-y-4">
          <h3 class="text-sm font-medium text-gray-900 uppercase tracking-wider">Settings</h3>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex items-center">
              <label class="flex items-center space-x-3 cursor-pointer">
                <input v-model="enabled" type="checkbox" class="w-5 h-5 text-indigo-600 rounded" />
  ...
  >
    <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 p-6">
      <h3 class="text-lg font-bold text-gray-900 mb-4">Policy Preview</h3>
      <p class="text-gray-600 mb-4">
        This policy will affect approximately
        <strong
          >all users matching "{{
            sourceTypeOptions.find(o => o.value === sourceType)?.label
          }}"</strong
        >
        criteria.
      </p>
      <div class="space-y-2 mb-6">
        <div class="flex justify-between text-sm">
          ...
          <span class="font-medium">{{
            targets.filter(t => t.target_type === 'team').length
          }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-gray-500">Target Channels:</span>
          <span class="font-medium">{{
            targets.filter(t => t.target_type === 'channel').length
          }}</span>
        </div>
      </div>
