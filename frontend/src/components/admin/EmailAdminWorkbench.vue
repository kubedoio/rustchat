...
const selectedFamily = computed(
  () => templateFamilies.value.find(f => f.id === selectedFamilyId.value) || null
)
...
async function loadOutbox() {
  ...
  if (outboxFilters.value.recipient_email)
    params.recipient_email = outboxFilters.value.recipient_email
...
async function testProvider(id: string) {
  ...
  try {
    ...
    providerTestStatus.value = {
      ...providerTestStatus.value,
      [id]: extractError(e, 'Provider test failed'),
    }
  }
}
...
<template>
  <div class="space-y-6">
    <div class="rounded-xl border border-border-1 bg-bg-surface-1 p-5">
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-text-3 mt-1">
            Manage providers, workflow routing, templates, outbox, and event history.
          </p>
        </div>
        <button
          @click="refreshAll"
      ...
      <div
        v-if="error"
        class="mt-4 rounded-lg border border-danger/20 bg-danger/10 px-3 py-2 text-sm text-danger"
      >
        {{ error }}
      </div>
      <div
        v-else-if="notice"
        class="mt-4 rounded-lg border border-success/20 bg-success/10 px-3 py-2 text-sm text-success"
      >
        {{ notice }}
      </div>
              ...
              <tr v-for="provider in providers" :key="provider.id" class="border-t border-border-1">
                <td class="py-3 pr-4">
                  ...
                  <div class="text-xs text-text-3">
                    {{ provider.tls_mode }} · {{ provider.username || 'no auth' }}
                  </div>
                </td>
                <td class="py-3 pr-4">
                ...
                <td class="py-3 pr-4">
                  ...
                  <span
                    ...
                    class="ml-2 px-2 py-1 rounded text-xs bg-warning/10 text-warning"
                    >Default</span
                  >
                </td>
                <td class="py-3 pr-4">
                  <div class="text-xs text-text-3 min-h-4">
    ...
    <div
      ...
      class="rounded-xl border border-border-1 bg-bg-surface-1 p-5"
    >
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="text-left text-text-3">
            ...
            <tr v-for="wf in workflows" :key="wf.id" class="border-t border-border-1">
              ...
              <td class="py-3 pr-4">
                <label class="inline-flex items-center gap-2">
                  <input
                    v-model="wf.enabled"
                    type="checkbox"
                    :disabled="!wf.can_disable"
                    class="w-4 h-4 rounded"
                  />
                  <span class="text-xs text-text-3">{{
                    wf.can_disable ? 'Editable' : 'Required'
                  }}</span>
                </label>
              </td>
              <td class="py-3 pr-4">
                <input
                  v-model="wf.default_locale"
                  class="w-20 px-2 py-1 rounded border border-border-2 bg-bg-surface-1"
                />
              </td>
              <td class="py-3 pr-4">
                <select
                  v-model="wf.selected_template_family_id"
                  class="w-56 px-2 py-1 rounded border border-border-2 bg-bg-surface-1"
                >
                  <option :value="null">Default / none</option>
                  <option v-for="f in templateFamilies" :key="f.id" :value="f.id">
                    {{ f.key }}
                  </option>
                </select>
              </td>
              <td class="py-3 pr-4">
                <details class="max-w-sm">
                  ...
                  <pre class="mt-2 p-2 rounded bg-bg-surface-2 text-text-1 text-xs overflow-auto">{{
                    JSON.stringify(wf.policy, null, 2)
                  }}</pre>
                </details>
              </td>
              <td class="py-3 text-right">
    ...
    <div v-else-if="activeTab === 'templates'" class="space-y-6">
      <div class="rounded-xl border border-border-1 bg-bg-surface-1 p-5">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3">
          <input
            v-model="familyForm.key"
            placeholder="family_key"
          ...
          />
          <input
            v-model="familyForm.workflow_key"
            placeholder="workflow_key (optional)"
            class="px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
          />
          <button
        ...
        <textarea
          ...
          class="mt-3 w-full px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
        />
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <tr
                v-for="family in templateFamilies"
                :key="family.id"
              ...
              >
                ...
                <td class="py-3 pr-4">
                  <template v-if="editingFamilyId === family.id && !family.is_system">
                    <input
                      v-model="editableFamilyDraft(family).name"
                      class="w-full px-2 py-1 rounded border border-border-2 bg-bg-surface-1"
                    />
                    <textarea
                      v-model="editableFamilyDraft(family).description"
                      rows="2"
                      class="mt-2 w-full px-2 py-1 rounded border border-border-2 bg-bg-surface-1"
                    />
                  </template>
                  <template v-else>
                ...
                <td class="py-3 pr-4">
                  <span
                    class="px-2 py-1 rounded text-xs"
                    :class="
                      family.is_system ? 'bg-bg-surface-2 text-text-2' : 'bg-brand/10 text-brand'
                    "
                  >
      ...
      <div class="rounded-xl border border-border-1 bg-bg-surface-1 p-5" v-if="selectedFamily">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              v-model="versionForm.locale"
              class="w-full px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
            />
          </div>
          <div>
            <label class="block text-sm mb-1 text-text-2">Subject</label>
            <input
              v-model="versionForm.subject"
              class="w-full px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
            />
          </div>
        </div>
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              ...
              <tr v-if="templateVersions.length === 0">
                <td colspan="6" class="py-4 text-sm text-text-3">
                  No versions yet for this family.
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-else-if="activeTab === 'outbox'" class="space-y-6">
      <div class="rounded-xl border border-border-1 bg-bg-surface-1 p-5">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          <input
            v-model="outboxFilters.status"
            placeholder="status (queued/failed/...)"
            class="px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
          />
          <input
            v-model="outboxFilters.workflow_key"
            placeholder="workflow_key"
            class="px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
          />
          <input
            v-model="outboxFilters.recipient_email"
    ...
    <div v-else-if="activeTab === 'events'" class="space-y-6">
      <div class="rounded-xl border border-border-1 bg-bg-surface-1 p-5">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input
            v-model="eventFilters.workflow_key"
            placeholder="workflow_key"
            class="px-3 py-2 rounded-lg border border-border-2 bg-bg-surface-1"
          />
          <input
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <tr v-for="event in events" :key="event.id" class="border-t border-border-1">
                ...
                <td class="py-3 pr-4 text-xs text-danger">
