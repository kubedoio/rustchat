...
<template>
  <div v-if="parentMessage" class="flex-1 flex flex-col min-h-0 bg-surface">
      ...
      <div class="flex items-start space-x-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-baseline space-x-2 mb-1">
            <span class="font-bold text-[15px] text-gray-900 leading-tight">{{
          ...
          <!-- Parent Files -->
          <div
            v-if="parentMessage.files && parentMessage.files.length > 0"
            class="mt-4 flex flex-wrap gap-2"
          >
            <template v-for="file in parentMessage.files" :key="file.id">
              <FilePreview
                :file="file"
                @preview="f => openGallery(f, parentMessage!.files || [])"
              />
            </template>
          </div>
        </div>
    ...
    <div
      ...
      class="px-5 py-3 border-b border-border-dim text-[11px] font-bold text-gray-500 uppercase tracking-widest bg-surface/50"
    >
    ...
    <div class="p-4 border-t border-border-dim bg-surface-dim/30">
      <div
        class="flex items-end space-x-2 bg-surface border border-border-dim rounded-xl focus-within:ring-2 focus-within:ring-primary/40 focus-within:border-primary/50 transition-all p-1.5 shadow-sm"
      >
        <textarea
