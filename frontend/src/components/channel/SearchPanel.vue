...
async function handleSearch() {
  ...
  loading.value = true
  try {
    searchResults.value = await messageStore.searchMessages(props.channelId, searchQuery.value)
  } catch (e) {
  ...
  <div class="flex-1 flex flex-col min-h-0 bg-bg-surface-1">
    ...
    <div class="p-sp-4 border-b border-border-1 bg-bg-surface-2/30">
      <div class="relative group">
        <Search
          class="absolute left-sp-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-3 group-focus-within:text-brand transition-standard"
        />
        <input
          v-model="searchQuery"
          type="text"
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar">
      <div v-if="loading" class="text-center py-12 text-gray-500">
        <div
          class="animate-spin w-6 h-6 border-2 border-primary border-t-transparent rounded-full mx-auto mb-3"
        ></div>
      ...
      <div
        ...
        class="text-center py-12 text-gray-500 px-6"
      >
        <div
          class="w-16 h-16 bg-surface-dim rounded-full flex items-center justify-center mx-auto mb-4 border border-border-dim"
        >
        ...
        >
          <div class="flex items-start justify-between mb-sp-1">
            <div class="flex items-center space-x-sp-2">
              <span
                class="font-semibold text-sm text-text-1 hover:text-brand transition-standard"
                >{{ message.username }}</span
              >
              <span class="text-xs text-text-3 font-medium tracking-tight">{{
                format(new Date(message.timestamp), 'MMM d, h:mm a')
              }}</span>
            </div>
            <div
              class="opacity-0 group-hover:opacity-100 transition-standard transform translate-x-1 group-hover:translate-x-0"
            >
              <button
                class="text-text-3 hover:text-brand p-1.5 rounded-r-1 hover:bg-brand/5 transition-standard"
