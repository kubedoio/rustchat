...
} from '../../features/presence/presencePresentation'
  ...
  <div class="flex h-full flex-col bg-bg-surface-1">
    ...
    <div class="space-y-4 border-b border-border-1 bg-bg-surface-2/60 p-4">
      <div class="relative group">
        <Search
          class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-3 transition-colors group-focus-within:text-brand"
        />
        <input
          v-model="searchQuery"
          type="text"
      ...
      <button
        class="flex w-full items-center justify-center space-x-2 rounded-r-2 bg-brand/10 px-3 py-2 text-sm font-semibold text-brand transition-standard active:scale-[0.98] hover:bg-brand/15"
      >
    ...
    <div class="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-1 flex items-center gap-2 text-xs text-text-3">
                ...
                <span
                  ...
                  :class="getPresenceMeta(member.presence).badgeClass"
                >
      ...
      <div v-if="loading" class="flex flex-col items-center space-y-3 py-10">
        <div
          class="h-6 w-6 animate-spin rounded-full border-2 border-brand border-t-transparent font-medium uppercase tracking-widest"
        ></div>
        <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-text-3">
          Fetching Members
        </p>
      </div>
      ...
      <div v-if="!loading && filteredMembers.length === 0" class="space-y-4 px-6 py-16 text-center">
        <div
          class="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-border-1 bg-bg-surface-2 opacity-60"
        >
