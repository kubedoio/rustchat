...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <!-- Modal -->
      <div
        class="relative mx-4 max-h-[80vh] w-full max-w-lg overflow-hidden rounded-r-3 border border-border-1 bg-bg-surface-1 shadow-2xl"
      >
        <!-- Header -->
        <div class="flex items-center justify-between border-b border-border-1 px-6 py-4">
          <div class="flex items-center space-x-3">
            <div
              class="flex h-10 w-10 items-center justify-center rounded-r-2 bg-brand/10 text-brand"
            >
              <Users class="h-5 w-5" />
            </div>
            <div>
              ...
              <p class="text-xs text-text-3">
                Join another workspace without leaving your current flow.
              </p>
            </div>
          </div>
          <button
        ...
        <div class="max-h-[60vh] overflow-y-auto p-6">
          <div v-if="teamStore.loading" class="py-8 text-center text-text-3">Loading teams...</div>

          <div
            v-else-if="availableTeams.length === 0 && myTeams.length === 0"
          ...
          <div v-else class="space-y-6">
            ...
            <div v-if="availableTeams.length > 0">
              <h3 class="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-text-3">
                Available to Join
              </h3>
              <div class="space-y-2">
                <div
