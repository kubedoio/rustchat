...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <!-- Modal -->
      <div
        class="relative bg-bg-surface-1 rounded-xl shadow-2xl border border-border-1 w-full max-w-md overflow-hidden"
      >
        <!-- Header -->
        ...
        <form class="p-5 space-y-4" @submit.prevent="submit">
          ...
          <div v-if="user" class="mb-4 p-3 bg-bg-surface-2 rounded-lg flex items-center gap-3">
            <div
              class="w-10 h-10 rounded-full bg-brand/10 flex items-center justify-center text-brand font-bold text-sm shrink-0"
            >
