...
async function handleDelete() {
  if (!props.channel || !canManageCurrentChannel.value) return
  if (!confirm(`Are you sure you want to delete #${props.channel.name}? This cannot be undone.`))
    return
  ...
  <Teleport to="body">
    <div v-if="isOpen && channel" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <!-- Tabs -->
        <div v-if="canManageCurrentChannel" class="flex border-b border-border-1 px-6 shrink-0">
          <button
            v-for="tab in tabs"
            :key="tab.id"
