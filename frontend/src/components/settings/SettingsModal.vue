...
watch(
  ...
  isOpen => {
    if (isOpen) {
      error.value = ''
      success.value = ''
      activeTab.value = allTabs.some(tab => tab.id === ui.settingsTab)
        ? ui.settingsTab
        : 'notifications'
    }
  }
  ...
  >
    ...
    >
      ...
      >
        <!-- Header -->
        <div
          class="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-border-1 shrink-0"
        >
        ...
        <div class="flex-1 min-h-0 flex flex-col sm:flex-row overflow-hidden">
          ...
          >
            <div class="border-b border-border-1 px-4 py-4">
              <p class="text-[10px] font-semibold uppercase tracking-[0.2em] text-text-3">
                Personal settings
              </p>
              <p class="mt-2 truncate text-sm font-semibold text-text-1">
                {{ auth.user?.display_name || auth.user?.username || 'Account' }}
              </p>
            ...
            <!-- Plugin Section -->
            <div
              class="px-3 pt-0 pb-2 sm:px-4 sm:py-2 text-[10px] font-semibold uppercase tracking-wider text-text-3"
            >
              Plugin Preferences
            </div>
            <nav
              class="grid grid-cols-2 gap-2 px-3 pb-3 sm:flex sm:flex-col sm:gap-0.5 sm:px-2 sm:pb-2"
            >
              <button
          ...
          <div class="flex-1 min-w-0 overflow-y-auto p-4 sm:p-6 bg-bg-surface-1">
            ...
            <div
              v-if="error"
              class="mb-4 p-3 bg-danger/10 border border-danger/20 rounded-r-2 text-danger text-sm"
            >
              {{ error }}
            </div>
            <div
              v-if="success"
              class="mb-4 p-3 bg-success/10 border border-success/20 rounded-r-2 text-success text-sm"
            >
              {{ success }}
            </div>
