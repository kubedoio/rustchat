...
const themeLabel = computed(() => THEME_OPTIONS.find(t => t.id === themeStore.theme)?.label || themeStore.theme)

const NAME_LABELS: Record<string, string> = { username: 'Show username', nickname: 'Show nickname', full_name: 'Show full name' }
const teammateNameDisplayLabel = computed(() => NAME_LABELS[preferencesStore.preferences?.teammate_name_display || 'username'] || 'Show username')
...
const messageDisplayLabel = computed(() => preferencesStore.preferences?.message_display === 'compact' ? 'Compact' : 'Standard')
const channelDisplayLabel = computed(() => preferencesStore.preferences?.channel_display_mode === 'centered' ? 'Centered' : 'Full width')

const LANGUAGE_LABELS: Record<string, string> = {
  en: 'English', es: 'Español', fr: 'Français', de: 'Deutsch', ja: '日本語', ko: '한국어',
  'pt-BR': 'Português (Brasil)', ru: 'Русский', 'zh-CN': '中文 (简体)', 'zh-TW': '中文 (繁體)'
}
const languageLabel = computed(() => LANGUAGE_LABELS[preferencesStore.preferences?.language || 'en'] || 'English')
...
function radioSetting(
  id: string, label: string, minDesc: string, maxDesc: string,
  minValue: () => string, model: Ref<any>, options: unknown[], save: () => void | Promise<void>
): DisplaySetting {
  return { id, label, minDescription: minDesc, maxDescription: maxDesc, minValue, editor: RadioSettingEditor, editorProps: { options }, model, save }
}

function toggleSetting(
  id: string, label: string, minDesc: string, maxDesc: string,
  minValue: () => string, model: Ref<boolean>, title: string, description: string, save: () => void | Promise<void>
): DisplaySetting {
  return { id, label, minDescription: minDesc, maxDescription: maxDesc, minValue, editor: ToggleSettingEditor, editorProps: { title, description }, model, save }
}

function selectSetting(
  id: string, label: string, minDesc: string, maxDesc: string,
  minValue: () => string, model: Ref<string>, options: unknown[], save: () => void | Promise<void>
): DisplaySetting {
  return { id, label, minDescription: minDesc, maxDescription: maxDesc, minValue, editor: SelectSettingEditor, editorProps: { options }, model, save }
}

const displaySettings = reactive<DisplaySetting[]>([
  radioSetting('font_size', 'Font Size', 'Adjust the text size for better readability', 'Choose your preferred text size',
    () => fontSizeLabel.value, localFontSize, [
      { value: 13, label: 'Small', description: 'Compact text for more content on screen', style: { fontSize: '13px' } },
      { value: 14, label: 'Medium', description: 'Standard text size', style: { fontSize: '14px' } },
      { value: 16, label: 'Large', description: 'Larger text for improved readability', style: { fontSize: '16px' } },
    ], () => { themeStore.setChatFontSize(localFontSize.value); expandedRow.value = null }),
  radioSetting('threaded_discussions', 'Threaded Discussions', 'Display replies in threads', 'Choose how to display thread replies',
    () => localCollapsedReplyThreads.value ? 'Collapsed' : 'Expanded', localCollapsedReplyThreads, [
      { value: false, label: 'Expanded', description: 'Show all replies in the channel' },
      { value: true, label: 'Collapsed', description: 'Show only the number of replies in the channel' },
    ], () => savePreference('threaded_discussions', { collapsed_reply_threads: localCollapsedReplyThreads.value })),
  radioSetting('clock_display', 'Clock Display', 'Select your preferred clock format', 'Choose your preferred time format',
    () => localUseMilitaryTime.value ? '24-hour clock' : '12-hour clock', localUseMilitaryTime, [
  ...
  radioSetting('teammate_name', 'Teammate Name Display', 'Select how teammate names are displayed', 'Choose how to display names for teammates',
    () => teammateNameDisplayLabel.value, localTeammateNameDisplay, [
  ...
  radioSetting('availability_badges', 'Online Availability Badges', 'Show online availability badges on profile images', 'Control visibility of online status indicators',
    () => localAvailabilityVisible.value ? 'Show' : 'Hide', localAvailabilityVisible, [
    ...
    ], () => savePreference('availability_badges', { availability_status_visible: localAvailabilityVisible.value })),
  toggleSetting('last_active', 'Share Last Active Time', 'Allow teammates to see when you were last active', 'Control whether others can see your last activity',
    () => localShowLastActive.value ? 'On' : 'Off', localShowLastActive,
    'Share last active time', 'Teammates can see when you were last online',
    () => savePreference('last_active', { show_last_active_time: localShowLastActive.value })),
  toggleSetting('link_previews', 'Link Previews', 'Show previews for links in messages', 'Control link preview generation',
    () => localLinkPreviews.value ? 'On' : 'Off', localLinkPreviews,
    'Show link previews', 'Display previews when links are posted',
    () => savePreference('link_previews', { link_previews_enabled: localLinkPreviews.value })),
  toggleSetting('image_previews', 'Image Previews', 'Show previews for images in messages', 'Control image preview display',
    () => localImagePreviews.value ? 'On' : 'Off', localImagePreviews,
    'Show image previews', 'Display image previews in messages',
    () => savePreference('image_previews', { image_previews_enabled: localImagePreviews.value })),
  radioSetting('message_display', 'Message Display', 'Select your message display mode', 'Choose how messages appear in channels',
    () => messageDisplayLabel.value, localMessageDisplay, [
      { value: 'standard', label: 'Standard', description: 'Full message display with avatars' },
      { value: 'compact', label: 'Compact', description: 'Condensed view for more messages on screen' },
    ], () => savePreference('message_display', { message_display: localMessageDisplay.value })),
  toggleSetting('click_to_reply', 'Click to Open Threads', 'Click anywhere on a message to open the reply thread', 'Control thread opening behavior',
    () => localClickToReply.value ? 'On' : 'Off', localClickToReply,
    'Click to open threads', 'Click on any message to view its thread',
    () => savePreference('click_to_reply', { click_to_reply: localClickToReply.value })),
  radioSetting('channel_display', 'Channel Display', 'Select your channel display mode', 'Choose how channel content is displayed',
    () => channelDisplayLabel.value, localChannelDisplay, [
  ...
  toggleSetting('quick_reactions', 'Quick Reactions', 'Show quick reaction buttons on messages', 'Control quick reaction buttons',
    () => localQuickReactions.value ? 'On' : 'Off', localQuickReactions,
    'Show quick reactions', 'Display emoji reaction buttons on hover',
    () => savePreference('quick_reactions', { quick_reactions_enabled: localQuickReactions.value })),
  toggleSetting('render_emoticons', 'Render Emoticons', 'Convert text emoticons to emoji', 'Convert text emoticons like :) to emoji',
    () => localRenderEmoticons.value ? 'On' : 'Off', localRenderEmoticons,
    'Render emoticons', 'Convert :) to 😊 and other text emoticons',
    () => savePreference('render_emoticons', { emoji_picker_enabled: localRenderEmoticons.value })),
  selectSetting('language', 'Language', 'Select your display language', 'Choose your display language',
    () => languageLabel.value, localLanguage, [
      { value: 'en', label: 'English' }, { value: 'es', label: 'Español' }, { value: 'fr', label: 'Français' },
      { value: 'de', label: 'Deutsch' }, { value: 'ja', label: '日本語' }, { value: 'ko', label: '한국어' },
      { value: 'pt-BR', label: 'Português (Brasil)' }, { value: 'ru', label: 'Русский' },
      { value: 'zh-CN', label: '中文 (简体)' }, { value: 'zh-TW', label: '中文 (繁體)' },
    ], () => savePreference('language', { language: localLanguage.value })),
])

const commonTimezones = ['UTC', 'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles',
  'America/Toronto', 'America/Vancouver', 'America/Mexico_City', 'America/Sao_Paulo', 'Europe/London',
  'Europe/Paris', 'Europe/Berlin', 'Europe/Madrid', 'Europe/Rome', 'Europe/Amsterdam', 'Europe/Moscow',
  'Asia/Tokyo', 'Asia/Shanghai', 'Asia/Hong_Kong', 'Asia/Singapore', 'Asia/Seoul', 'Asia/Mumbai',
