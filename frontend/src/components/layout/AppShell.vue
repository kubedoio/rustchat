...
<template>
  <div class="flex h-screen flex-col overflow-hidden bg-bg-app text-text-1 transition-standard">
    ...
    >
      <div v-if="showMobileSidebar" class="fixed top-0 left-0 bottom-0 z-50 flex shadow-2xl">
        <TeamRail class="h-full border-r border-border-1 bg-bg-surface-2" />
        <ChannelSidebar
          class="h-full border-r border-border-1 bg-bg-surface-2 w-[var(--sidebar-width)]"
        />
      </div>
    </Transition>
    ...
    <!-- Main Layout -->
    <div
      class="relative flex flex-1 overflow-hidden bg-[radial-gradient(circle_at_top,_color-mix(in_srgb,_var(--brand)_5%,transparent),transparent_38%)]"
    >
      <!-- Desktop Team Rail (Leftmost) -->
      <TeamRail v-if="!isMobile" class="border-r border-border-1 bg-bg-surface-2 shrink-0" />

      <!-- Desktop Channel Sidebar (LHS) -->
      <ChannelSidebar v-if="!isMobile" class="border-r border-border-1 bg-bg-surface-2 shrink-0" />
