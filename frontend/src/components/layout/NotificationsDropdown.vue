...
<template>
  <div
    class="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 overflow-hidden"
  >
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2 shrink-0">
              <span
                v-if="channel.mentions > 0"
                class="px-1.5 py-0.5 bg-red-500 text-white text-[10px] font-bold rounded-full"
              >
                {{ channel.mentions }}
              </span>
              <span
                class="px-1.5 py-0.5 bg-gray-200 text-gray-600 text-[10px] font-bold rounded-full"
              >
                {{ channel.count }}
              </span>
              <ArrowRight
                class="w-3.5 h-3.5 text-gray-300 group-hover:text-indigo-500 transition-colors"
              />
            </div>
          </div>
        </div>
