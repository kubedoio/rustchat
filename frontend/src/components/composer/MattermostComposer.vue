...
function applyTransform(
  transform: (sel: TextSelection) => { text: string; selectionStart: number; selectionEnd: number }
) {
...
// Toolbar actions
function onBold() {
  applyTransform(makeBold)
}
function onItalic() {
  applyTransform(makeItalic)
}
function onStrikethrough() {
  applyTransform(makeStrikethrough)
}
function onHeading() {
  applyTransform(sel => makeHeading(sel, 4))
}
function onLink() {
  applyTransform(makeLink)
}
function onInlineCode() {
  applyTransform(makeInlineCode)
}
function onQuote() {
  applyTransform(makeQuote)
}
function onBulletedList() {
  applyTransform(makeBulletedList)
}
function onNumberedList() {
  applyTransform(makeNumberedList)
}
...
function onEmojiGlyphSelect(emoji: string) {
  ...
  const newText =
    selection.text.substring(0, selection.selectionStart) +
    emoji +
...
function onSend() {
  ...
  const fileIds = attachedFiles.value.filter(f => f.uploaded).map(f => f.uploaded!.id)
  ...
  <div class="relative">
    ...
    >
      ...
      <div
        ...
        class="flex flex-wrap gap-2 p-3 border-b border-border-1"
      >
        <div
