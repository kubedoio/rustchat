...
<template>
  <div
    ref="rootRef"
    class="relative flex items-center gap-1 overflow-x-auto border-b border-border-1 bg-bg-surface-2/50 px-2 py-1 whitespace-nowrap"
  >
    <!-- Formatting buttons -->
    <button
    ...
    >
      ...
      <p class="mt-2 text-[10px] text-text-3">
        Use <code>:emoji:</code>, <code>@mention</code>, <code>~channel</code>, and
        <code>^k</code> (or Ctrl/Cmd+K) for command menu.
      </p>
    </div>
  </div>
</template>
