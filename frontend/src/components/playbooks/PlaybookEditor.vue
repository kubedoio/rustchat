...
async function save() {
  ...
  try {
    ...
    const payload = {
      ...form.value,
      keyword_triggers: form.value.keyword_triggers
        .split(',')
        .map(s => s.trim())
        .filter(s => s.length > 0),
    }
    ...
    <div class="flex-1 overflow-y-auto p-6 max-w-4xl mx-auto w-full">
      <div class="space-y-8">
          ...
          <div class="space-y-4">
            <div class="flex space-x-4">
              <div class="w-20">
                ...
                <input
                  v-model="form.icon"
                  class="w-full text-center text-2xl px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1"
                />
              </div>
              <div class="flex-1">
          ...
          <div class="space-y-4">
            <div class="flex items-center space-x-2">
              ...
              <label for="is_public" class="text-sm font-medium text-text-2"
                >Public (visible to everyone in team)</label
              >
            </div>
            ...
            <div class="border-t border-border-1 pt-4">
              <div class="flex items-center space-x-2 mb-2">
                ...
                <label for="create_channel" class="text-sm font-medium text-text-2"
                  >Create new channel on run</label
                >
              </div>
              ...
              <div v-if="form.create_channel_on_run" class="ml-6">
                ...
                >
                <input
                  v-model="form.channel_name_template"
                  type="text"
            ...
            <div class="border-t border-border-1 pt-4">
              ...
              <input
                v-model="form.keyword_triggers"
                type="text"
                placeholder="e.g. incident, outage, server down"
                class="w-full px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1"
              />
              <p class="text-xs text-text-3 mt-1">
                Comma separated keywords that will trigger a prompt to start this playbook.
              </p>
            </div>
          </div>
        </section>
        ...
        <section class="space-y-6">
          ...
          <div
            v-for="(checklist, cIndex) in checklists"
            :key="checklist.id"
          ...
          >
            ...
            <div class="p-4 space-y-2">
              <div
                v-for="(task, tIndex) in checklist.tasks"
                :key="task.id"
              ...
              >
                ...
                <div class="flex-1 space-y-1">
                  <input
                    v-model="task.title"
                    class="w-full bg-transparent border-b border-transparent hover:border-border-1 focus:border-primary focus:outline-none py-1 text-text-1"
                    placeholder="Task title..."
                  />
                  <input
                    v-model="task.description"
                    class="w-full text-sm bg-transparent border-none focus:ring-0 p-0 text-text-3"
