...
<template>
    <div class="h-full flex flex-col bg-bg-surface-2">
        <!-- Header -->
        <header class="bg-bg-surface-1 border-b border-border-1 px-6 py-4 flex justify-between items-center">
        ...
        <div class="flex-1 overflow-y-auto p-6 max-w-4xl mx-auto w-full">
            <div class="space-y-8">
                <!-- General Settings -->
                <section class="bg-bg-surface-1 rounded-xl border border-border-1 p-6 shadow-sm">
                    <h2 class="text-lg font-semibold mb-4 text-text-1">General Information</h2>
                    <div class="space-y-4">
                        <div class="flex space-x-4">
                            <div class="w-20">
                                <label class="block text-sm font-medium text-text-2 mb-1">Icon</label>
                                <input v-model="form.icon" class="w-full text-center text-2xl px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1" />
                            </div>
                            <div class="flex-1">
                                <label class="block text-sm font-medium text-text-2 mb-1">Name</label>
                                <input v-model="form.name" type="text" placeholder="e.g. Incident Response" class="w-full px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1" />
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-text-2 mb-1">Description</label>
                            <textarea v-model="form.description" rows="2" class="w-full px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1"></textarea>
                        </div>
                    </div>
                </section>
                ...
                <!-- Automation Settings -->
                <section class="bg-bg-surface-1 rounded-xl border border-border-1 p-6 shadow-sm">
                    <h2 class="text-lg font-semibold mb-4 text-text-1">Automation & Settings</h2>
                    <div class="space-y-4">
                        <div class="flex items-center space-x-2">
                            <input v-model="form.is_public" type="checkbox" id="is_public" class="rounded border-border-2 text-primary focus:ring-brand" />
                            <label for="is_public" class="text-sm font-medium text-text-2">Public (visible to everyone in team)</label>
                        </div>

                        <div class="border-t border-border-1 pt-4">
                            <div class="flex items-center space-x-2 mb-2">
                                <input v-model="form.create_channel_on_run" type="checkbox" id="create_channel" class="rounded border-border-2 text-primary focus:ring-brand" />
                                <label for="create_channel" class="text-sm font-medium text-text-2">Create new channel on run</label>
                            </div>
                            
                            <div v-if="form.create_channel_on_run" class="ml-6">
                                <label class="block text-sm font-medium text-text-2 mb-1">Channel Name Template</label>
                                <input v-model="form.channel_name_template" type="text" placeholder="incident-{{date}}" class="w-full px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1" />
                                <p class="text-xs text-text-3 mt-1" v-pre>Available variables: {{date}}, {{playbook_name}}</p>
                            </div>
                        </div>

                        <div class="border-t border-border-1 pt-4">
                            <label class="block text-sm font-medium text-text-2 mb-1">Keyword Triggers</label>
                            <input v-model="form.keyword_triggers" type="text" placeholder="e.g. incident, outage, server down" class="w-full px-3 py-2 border border-border-2 rounded-lg bg-bg-surface-1 text-text-1" />
                            <p class="text-xs text-text-3 mt-1">Comma separated keywords that will trigger a prompt to start this playbook.</p>
                        </div>
                    </div>
                </section>
                ...
                <section class="space-y-6">
                    ...
                    <div v-for="(checklist, cIndex) in checklists" :key="checklist.id" 
                         class="bg-bg-surface-1 rounded-xl border border-border-1 overflow-hidden shadow-sm">
                        <!-- Checklist Header -->
                        <div class="bg-bg-surface-2 p-4 flex items-center space-x-3 border-b border-border-1">
                            <GripVertical class="w-5 h-5 text-text-3 cursor-move" />
                            <input v-model="checklist.name" class="flex-1 bg-transparent font-medium text-text-1 focus:outline-none focus:ring-2 focus:ring-primary rounded px-2" placeholder="Checklist Name" />
                        ...
                        <div class="p-4 space-y-2">
                            <div v-for="(task, tIndex) in checklist.tasks" :key="task.id" class="flex items-start space-x-3 group">
                                <CheckSquare class="w-5 h-5 text-text-3 mt-2" />
                                <div class="flex-1 space-y-1">
                                    <input v-model="task.title" class="w-full bg-transparent border-b border-transparent hover:border-border-1 focus:border-primary focus:outline-none py-1 text-text-1" placeholder="Task title..." />
                                    <input v-model="task.description" class="w-full text-sm bg-transparent border-none focus:ring-0 p-0 text-text-3" placeholder="Description (optional)" />
                                </div>
                                <button @click="removeTask(cIndex, tIndex)" class="opacity-0 group-hover:opacity-100 text-text-3 hover:text-danger p-1">
                                    <X class="w-4 h-4" />
                                </button>
                            </div>
                            
                            <button @click="addTask(cIndex)" class="flex items-center text-sm text-text-3 hover:text-primary mt-2 px-1">
