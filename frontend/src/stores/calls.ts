...
export const useCallsStore = defineStore('calls', () => {
  ...
  const preferredAudioOutput = ref<string>(
    localStorage.getItem('calls_preferred_audio_output') || ''
  )
  const preferredVideoDevice = ref<string>(
    localStorage.getItem('calls_preferred_video_device') || ''
  )
  ...
  function syncSelfCallFlags(call: CallState) {
    ...
    isScreenSharing.value =
      (call.screen_sharing_id_raw || call.screen_sharing_id) === authStore.user?.id
  }
    ...
    try {
      ...
      pc.ontrack = event => {
        ...
        if (event.streams && event.streams[0]) {
          ...
          const hasTrack = synthetic.getTracks().some(track => track.id === event.track.id)
          if (!hasTrack) {
        ...
        event.track.onended = () => {
          ...
          for (const [key, stream] of call.remoteStreams.entries()) {
            const remainingTracks = stream.getTracks().filter(track => track.id !== event.track.id)
