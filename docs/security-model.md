...
- OAuth/SAML SSO is supported via configurable providers
...
Query-string token transport is disabled by default in production.

### Session Storage

Active sessions and rate-limit state are stored in Redis. PostgreSQL is the source of truth for users, teams, and permissions.
...
- [ ] Disable query-string WebSocket tokens: `RUSTCHAT_SECURITY_WS_ALLOW_QUERY_TOKEN=false`
